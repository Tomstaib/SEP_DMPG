...
def add_logging_level(level_name, level_num, method_name=None) -> None:
    ...
    # Check if the logging level or method name already exists before adding
    if hasattr(logging, level_name):
        # Optional: log or print that the level is already defined
        print(f'Logging level {level_name} already defined in logging module')
        return  # Exit the function if the level is already defined

    if hasattr(logging, method_name):
        print(f'Logging method {method_name} already defined in logging module')
        return  # Exit if the method is already defined

    if hasattr(logging.getLoggerClass(), method_name):
        print(f'Logging method {method_name} already defined in logger class')
        return  # Exit if the method is already defined

    # Define the new log level and its methods if not already present
...
# Example usage of the add_logging_level function to add TRACE level
try:
    add_logging_level('TRACE', logging.DEBUG + 5)
except AttributeError as e:
    print(f"Logging level 'TRACE' is already defined: {e}")
