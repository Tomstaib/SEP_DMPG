...
class TestCases(unittest.TestCase):
    ...
    def test_sequential_replications(self):
        pivot_table2 = run_replications(model=setup_model4_1, minutes=1440, num_replications=10, multiprocessing=False)

        self.assertEqual(pivot_table2.at[('Entity', 'Entity', 'AvgTimeInSystem'), 'Minimum'], 4.5494)
        self.assertEqual(pivot_table2.at[('Entity', 'Entity', 'AvgTimeInSystem'), 'Average'], 6.3527)
        self.assertEqual(pivot_table2.at[('Entity', 'Entity', 'AvgTimeInSystem'), 'Maximum'], 9.0393)
        self.assertEqual(pivot_table2.at[('Entity', 'Entity', 'AvgTimeInSystem'), 'Half-Width'], 0.7419)

    def test_parallel_replications(self):
        pivot_table2 = run_replications(model=setup_model4_1, minutes=1440, num_replications=10, multiprocessing=True)
