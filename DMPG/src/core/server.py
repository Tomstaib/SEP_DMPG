...
class Server(ResetAbleNamedObject, RoutingObject):
    ...
    def __init__(self, env: simpy.Environment, name: str,
       ...
       """
        ...
        self.unscheduled_time_last_scheduling_period = 0
        ...
        self.production_time_per_scheduling_period = 0
    ...
    def _process_entity_logic(self, entity):
        ...
        self.production_time_per_scheduling_period += processing_time   # TODO: Consider breakdowns
    ...
    def get_utilization_last_scheduling_period(self, reset_utilization=True):
        if self.env.now == 0:
            return 0
        if self.production_time_per_scheduling_period == 0:
            return 0
        utilization = self.scheduling_period / self.production_time_per_scheduling_period
        if reset_utilization:
            self.production_time_per_scheduling_period = 0
        """
        elif self.unscheduled_time_last_scheduling_period == 0:
            return 100
        utilization = 100 - (self.unscheduled_time_last_scheduling_period / self.scheduling_period * 100)
        if utilization < 0:
            print("test")
        if reset_utilization:
            self.unscheduled_time_last_scheduling_period = 0
