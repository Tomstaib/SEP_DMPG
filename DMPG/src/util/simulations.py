...
from src.core.separator import Separator
...
def replace_q_net(scheduler, new_q_net, epsilon):
    scheduler.q_net = new_q_net
    scheduler.epsilon_scheduler.epsilon = epsilon
    scheduler.initialize()
    return


def run_rl_train_simulation(model: Callable, minutes: Union[int, float],
                            params, q_net, epsilon, queue,) -> pd.DataFrame:

    random.seed(RANDOM_SEED)
    env = simpy.Environment()

    model(env, params)
    scheduler = Source.sources.resetable_named_objects[0].scheduler
    replace_q_net(scheduler, q_net, epsilon)

    env.run(until=minutes)

    Source.sources.resetable_named_objects[0].scheduler.get_buffer_data(queue)
    processing_time = Server.servers.resetable_named_objects[0].total_processing_time_pivot_table
    reset_simulation()
...
def run_simulation(model: Callable, minutes: Union[int, float], warm_up: Union[int, float] = None,
    ...
    pivot_table = get_statistics(env, store_pivot_in_file)

    return pivot_table


def get_statistics(env, store_pivot_in_file):
    # Get the statistics
    entity_stats, server_stats, combiner_stats, separator_stats, sink_stats, source_stats = calculate_statistics(env)
    ...
    if store_pivot_in_file:
        pivot_table.to_csv(store_pivot_in_file, sep=';', decimal=',')

    return pivot_table


def save_source_scheduling_model(env):
    for source in Source.sources:
        source.save_agent()


def get_model(source):
    q_net = source.scheduler.q_net
    replay_buffer = source.scheduler.replay_buffer
    epsilon_scheduler = source.scheduler.epsilon_scheduler
    return [q_net, replay_buffer, epsilon_scheduler]


def set_model(source, agent):
    source.scheduler.q_net = agent[0]
    source.scheduler.replay_buffer = agent[1]
    source.scheduler.epsilon_scheduler = agent[2]
    # source.scheduler.env.source = source
...
def calculate_statistics(env) -> Tuple:
    ...
    :return Tuple[Dict, List[Dict], Dict, Dict]:
    A tuple containing dictionaries for entity, server, sink, and source statistics.
    """
    ...
    # Caclculate combiner statistics
    combiner_stats = []
    for combiner in Combiner.combiners:
        ...
        if env.now > gi.DURATION_WARM_UP:
            scheduled_utilization_pivot_table = (
                (combiner.total_processing_time_pivot_table / current_simulation_time) * 100) \
                if current_simulation_time > 0 else 0
            avg_time_processing_pivot_table = (
                combiner.total_processing_time_pivot_table / combiner.entities_processed
                if combiner.entities_processed > 0 else 0)

        # Calculate Units Utilized
        units_utilized_pivot_table = calculate_units_utilized(combiner.units_utilized_over_time, combiner.capacity,
                                                              env.now)
        ...
        combiner_stats.append({
            ...
            'UnitsUtilized': units_utilized_pivot_table,
            'AvgTimeProcessing': avg_time_processing_pivot_table,
            'TotalTimeProcessing': combiner.total_processing_time_pivot_table,
            'NumberParentsEntered': combiner.number_entered_parent_pivot_table,
            'NumberParentsExited': combiner.number_exited_parent_pivot_table,
            'NumberMembersEntered': combiner.number_entered_member_pivot_table,
            'NumberMembersExited': combiner.number_exited_member_pivot_table,
            'NumberDowntimes': combiner.number_downtimes_pivot_table,
            'TotalDowntime': combiner.total_downtime_pivot_table
        })

    # Caclculate separator statistics
    separator_stats = []
    for separator in Separator.separators:
        ...
        if env.now > gi.DURATION_WARM_UP:
            scheduled_utilization_pivot_table = (
                (separator.total_processing_time_pivot_table / current_simulation_time) * 100) \
                if current_simulation_time > 0 else 0
            avg_time_processing_pivot_table = (
                separator.total_processing_time_pivot_table / separator.entities_processed
                if separator.entities_processed > 0 else 0)

        # Calculate Units Utilized
        units_utilized_pivot_table = calculate_units_utilized(separator.units_utilized_over_time, separator.capacity,
                                                              env.now)
        ...
        separator_stats.append({
            ...
            'UnitsUtilized': units_utilized_pivot_table,
            'AvgTimeProcessing': avg_time_processing_pivot_table,
            'TotalTimeProcessing': separator.total_processing_time_pivot_table,
            'NumberEntered': separator.number_entered_pivot_table,
            'NumberParentsExited': separator.number_exited_parent_pivot_table,
            'NumberMembersExited': separator.number_exited_member_pivot_table,
    ...
    return entity_stats, server_stats, combiner_stats, separator_stats, sink_stats, source_stats


def reset_simulation():
    EntityManager.destroy_all_entities()
    Source.sources.resetable_named_objects = []
    Server.servers.resetable_named_objects = []
    Sink.sinks.resetable_named_objects = []
...
def replication(env_setup_func, calculate_stats_func, minutes, r) -> pd.DataFrame:
    ...
    Combiner.combiners.reset_all()
    Separator.separators.reset_all()
...
def run_replications(model: Callable, minutes, num_replications, warm_up: Union[int, float] = None,
    ...
    combiner_stat_names = ['ScheduledUtilization', 'UnitsUtilized', 'AvgTimeProcessing',
                           'TotalTimeProcessing', 'NumberParentsEntered', 'NumberParentsExited', 'NumberMembersEntered',
                           'NumberMembersExited', 'TotalDowntime', 'NumberDowntimes']
    separator_stat_names = ['ScheduledUtilization', 'UnitsUtilized', 'AvgTimeProcessing',
                            'TotalTimeProcessing', 'NumberEntered', 'NumberParentsExited', 'NumberMembersExited',
                            'TotalDowntime', 'NumberDowntimes']
    ...
    def process_results(entity_stats, server_stats, combiner_stats, separator_stats, sink_stats, source_stats) -> None:
        """
        ...
        for combiner_stat in combiner_stats:
            combiner_name = combiner_stat['Combiner']
            all_combiner_stats.setdefault(combiner_name, []).append(combiner_stat)
        for separator_stat in separator_stats:
            separator_name = separator_stat['Separator']
            all_separator_stats.setdefault(separator_name, []).append(separator_stat)
    ...
    return create_pivot(all_entity_stats, all_server_stats, all_combiner_stats, all_separator_stats, all_sink_stats,
                        all_source_stats, entity_stat_names, server_stat_names, combiner_stat_names,
                        separator_stat_names, sink_stat_names, source_stat_names)
...
def create_pivot(all_entity_stats, all_server_stats, all_combiner_stats, all_separator_stats, all_sink_stats,
                 all_source_stats, entity_stat_names, server_stat_names, combiner_stat_names, separator_stat_names,
    ...
    def flatten_stats(stats, name, stat_names, is_entity=False) -> list:
        ...
        param: name (str): Name of the component type (entity, Server, Combiner, Separator, Sink, Source).
    ...
    aggregate_combiner_stats = {combiner_name: {key: calculate_aggregate_stats([stat[key] for stat in stats_list])
                                                for key in combiner_stat_names}
                                for combiner_name, stats_list in all_combiner_stats.items()}

    aggregate_separator_stats = {separator_name: {key: calculate_aggregate_stats([stat[key] for stat in stats_list])
                                                  for key in separator_stat_names}
                                 for separator_name, stats_list in all_separator_stats.items()}
