...
def visualize_system():
    ...
    for server in Server.servers:
        ...
        for next_component, prob in server.next_component:
...
def scatterplot(component_type, variable1, variable2):
...
def histogram(component_type, variable):
    ...
    for i, (name, values) in enumerate(histogram_data.items()):
        ax = axes[i]
        sns.histplot(values, bins='auto', kde=True, color=sns.cubehelix_palette(start=.5, rot=-.75)[i], ax=ax, alpha=0.7)
...
def boxplot(component_type, variable):
...
def violinplot(component_type, variable):
