...
def setup_model_pcb_with_arrival_table(env):
    # Create servers, sinks, and sources
    source1 = Source(env, "PCB", arrival_table_path="arrivalTableTest.csv")
...
def setup_model_pcb(env):
    # Create servers, sinks, and sources
    source1 = Source(env, "source", arrival_table_path='arrivalTableTest.csv')

    server1 = Server(env, "server", (random.triangular, 6, 10, 12))
